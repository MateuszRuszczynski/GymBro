...
class PublicAccessTest(TestCase):
    ...
    def test_exercise_list_redirects_if_not_logged_in(self):
        ...
        self.assertRedirects(response, "/accounts/login/?next=/exercises/")
...
class MuscleGroupViewTest(TestCase):
    ...
    def test_create_redirects_to_list(self):
        response = self.client.post(
            reverse("tracker:muscle-group-create"), {"name": "Legs"}
        )
...
class ExerciseViewTest(TestCase):
    def setUp(self):
        self.user = GymUser.objects.create_user(username="Andrew", password="test123")
    ...
    def test_exercise_detail_view(self):
        response = self.client.get(
            reverse("tracker:exercise-detail", kwargs={"pk": self.exercise.pk})
        )
        self.assertEqual(response.status_code, 200)
...
class WorkoutLogViewTest(TestCase):
    ...
    def test_detail_view(self):
        response = self.client.get(
            reverse("tracker:workout-log-detail", kwargs={"pk": self.log.pk})
        )
        self.assertEqual(response.status_code, 200)
